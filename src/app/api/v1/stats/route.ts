...
// GET /api/v1/stats — Dashboard statistics (case-centric view)
  ...
  try {
    ...
    const [totalCases, pendingCases, activeCases, closedCases, urgentCases, rejectedCases] =
      await Promise.all([
        db.case.count(),
        db.case.count({
          where: { status: { in: ["draft", "submitted", "verifying"] } },
        }),
        db.case.count({
          where: { status: { in: ["verified", "scoring", "scored", "approved", "disbursing", "disbursed", "follow_up"] } },
        }),
