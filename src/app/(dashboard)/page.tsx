...
// Hydration guard — defined at module level but only used inside component
const subscribe = () => () => {};
const getSnapshot = () => true;
const getServerSnapshot = () => false;

interface DashboardStats {
  cases: { total: number; pending: number; active: number; closed: number; urgent: number; rejected: number };
  ...
  recentCases: { id: string; caseNumber: string; applicantName: string; status: string; priority: string; createdAt: string }[];
  recentDonations: { id: string; donorName: string; amount: number; status: string; method: string; date: string }[];
}

const STATUS_STYLES: Record<string, { label: string; variant: "default" | "secondary" | "destructive" | "outline" }> = {
  draft: { label: "Draf", variant: "secondary" },
  submitted: { label: "Hantaran", variant: "outline" },
  verifying: { label: "Verifikasi", variant: "outline" },
  verified: { label: "Disahkan", variant: "default" },
  approved: { label: "Diluluskan", variant: "default" },
  disbursing: { label: "Pengagihan", variant: "default" },
  disbursed: { label: "Diagihkan", variant: "default" },
  follow_up: { label: "Susulan", variant: "outline" },
  closed: { label: "Ditutup", variant: "secondary" },
  rejected: { label: "Ditolak", variant: "destructive" },
  pending: { label: "Menunggu", variant: "outline" },
  confirmed: { label: "Sah", variant: "default" },
};
...
export default function DashboardPage() {
  // Hydration guard — hook call inside component
    ...
    <div className="space-y-6">
      ...
      >
        {[
          ...
          { title: "Baki Tersedia", value: formatCurrency(stats.disbursements.balance), icon: Wallet, description: `${stats.disbursements.totalTransactions} transaksi diagihkan`, color: "text-blue-600 dark:text-blue-400" },
          { title: "Program Aktif", value: stats.programmes.active, icon: Activity, description: `daripada ${stats.programmes.total} program`, color: "text-amber-600 dark:text-amber-400" },
        ].map((stat, i) => (
              ...
              {stats.recentCases.map((c) => {
                ...
                return (
                  <div key={c.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
                    ...
                    <Badge variant={statusStyle.variant} className="text-xs shrink-0">{statusStyle.label}</Badge>
                  </div>
                );
              })}
            </CardContent>
      ...
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7, duration: 0.3 }}>
        <Card>
          <CardHeader className="pb-3">
            ...
            <CardDescription>Semasa kes berdasarkan status</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center gap-2 overflow-x-auto pb-2">
              {[
                { label: "Draf", count: stats.cases.total - stats.cases.pending - stats.cases.active - stats.cases.closed - stats.cases.rejected, color: "bg-gray-400" },
